...
async function addDept() {
  ...
  const res = await pool.query('INSERT INTO department (name) VALUE ($1) RETURNING *', [deptName]);

  console.log(`New department added: ${res.rows[0].deptName}`);
  return res.rows[0];
}
...
async function addEmployee() {
  ...
  const roleChoices = depts.rows.map(({ id, title}) => ({ name: title, value: id }));

  // Selects employees to assign as manager
  const employees = await pool.quer('SELECT id, first_name, last_name FROM employee');
  const mgrChoices = depts.rows.map(({ id, first_name, last_name }) => ({ name: `${first_name} ${last_name}`, value: id}));

  // Getting new Employee details from user input
  const { newFirstName, newLastName, employeeRole, managerId } = await inquirer.prompt([
    {
      type: 'input',
      name: 'newFirstName',
      message: 'First Name:',
...
module.exports = { pg, viewDepts, viewRoles, viewEmployees, addDept, addRole, addEmployee };